...
import { VEHICLE_DATABASE, VEHICLE_COLORS, TUNING_CONFIG, type VehicleColor } from '../../features/vehicles/data/vehicleConfig';
import { calculateOverallCondition, getPerformanceMultiplier } from '../../features/vehicles/utils/vehicleWear';
import type { Vehicle } from '../../stores/useVehicleStore';

interface VehicleInfoMenuProps {
  vehicle: Vehicle | null;
  onClose: () => void;
}

export default function VehicleInfoMenu({ vehicle, onClose }: VehicleInfoMenuProps) {
  ...
  const condition = Math.round(calculateOverallCondition(vehicle as unknown as { model_id: string } | null));
  const mileage = Math.round((vehicle as { mileage?: number }).mileage || 0);
  const colorData = VEHICLE_COLORS.find((c: VehicleColor) => c.id === String((vehicle as { color: string }).color));
            ...
            <div className="flex items-center gap-4 mb-3">
              <div className="w-16 h-16 rounded-2xl bg-white/[0.03] border border-white/5 flex items-center justify-center shrink-0">
                <img 
                  ...
                  onError={(e: React.SyntheticEvent<HTMLImageElement>) => { e.currentTarget.src = '/car.png'; }}
                />
              </div>
