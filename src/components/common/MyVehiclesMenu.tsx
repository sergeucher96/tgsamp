...
import { VEHICLE_DATABASE, type VehicleConfig } from '../../features/vehicles/data/vehicleConfig';
import { useVehicleStore, type Vehicle } from '../../stores/useVehicleStore';
import { useTravelStore } from '../../stores/useTravelStore';
import { LOCATIONS } from '../../game/locations/locations';
import { calculateOverallCondition } from '../../features/vehicles/utils/vehicleWear';
import CarPreviewImage from '../ui/CarPreviewImage';
...
interface MyVehiclesMenuProps {
  onClose: () => void;
}

export default function MyVehiclesMenu({ onClose }: MyVehiclesMenuProps) {
  ...
  const getVehicleStatus = (vehicle: Vehicle): VehicleStatus => {
    ...
    if (vehicle.house_id) {
      const houseLoc = LOCATIONS.find((l: Location) => l.id === vehicle.house_id);
      if (houseLoc) {
  ...
  const handleGPS = (vehicle: Vehicle) => {
    const houseLoc = LOCATIONS.find((l: Location) => l.id === vehicle.house_id);
    if (houseLoc) {
          ...
          {myVehicles.length === 0 ? (
            ...
            <div className="p-4 space-y-3">
              {myVehicles.map((vehicle: Vehicle) => {
                const cfg = VEHICLE_DATABASE[vehicle.model_id] as VehicleConfig | undefined;
