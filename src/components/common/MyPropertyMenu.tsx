...
import { HOUSE_CLASSES, type HouseClassConfig } from '../../features/houses/data/houseConfig';
...
interface MyPropertyMenuProps {
  onClose: () => void;
}

export default function MyPropertyMenu({ onClose }: MyPropertyMenuProps) {
  ...
  const ownedHouses = (dbHouses || []).filter((h: House) => h.owner_id === player?.id);

  const handleEnterHouse = (house: House) => {
    if (!player) return;
  ...
  const handleGPS = (house: House) => {
    startRoute(house.id_name);
    onClose();
  };

  const isAtHouse = (house: House) => {
