// src/components/dev/HotspotTool.tsx
...
interface HotspotToolProps { onClose: () => void; onExport?: (data: Record<string, unknown>) => void; }

export default function HotspotTool({ onClose, onExport }: HotspotToolProps) {
      ...
      if (savedSubs) {
        try {
          ...
          if (parsed[subKey]) {
            ...
            subMusic = parsed[subKey].bgMusic || '';
            subVol = parsed[subKey].musicVolume ?? 0.5;
          }
        } catch (e) {}
      }
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              const reader = new FileReader();
              reader.onload = (ev) => {
                setActiveMusic(ev.target.result as string);
