...
interface ItemActionMenuProps {
  ...
  isHouse?: boolean;
  houseId?: string;
  onUse?: (item: ItemActionMenuProps['item']) => void;
  onTransfer?: (item: ItemActionMenuProps['item']) => void;
  onDrop?: (item: ItemActionMenuProps['item']) => void;
  onClose: () => void;
}
...
export default function ItemActionMenu({ item, location, onUse, onTransfer, onDrop, onClose }: ItemActionMenuProps) {
  const { items: dbItems } = useItemCategoryStore();

  const dbItem = dbItems.find((i: CategoryItem) => i.key === item.item_id || i.id?.toString() === item.item_id);
  const fallbackItem = ITEM_DATABASE[item.item_id];
  const itemData = dbItem
    ? { name: dbItem.name, icon: dbItem.icon || '📦', desc: dbItem.description || '', type: dbItem.type || 'item', action: dbItem.actions?.[0] || fallbackItem?.action }
    : fallbackItem;
