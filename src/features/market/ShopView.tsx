...
import { SHOPS_DATABASE } from '../businesses/data/shops';
import { CLOTHING_DATABASE } from '../character/data/clothingConfig';
import { EQUIPMENT_SLOTS } from '../character/data/clothingConfig';
...
export default function ShopView({ shopType, player, onClose }: ShopViewProps) {
  ...
  const getItemDisplay = (shopItem: ShopItem): ItemDisplay => {
  ...
  const getResourceCosts = (product: any): ResourceCost[] | null => {
    ...
    return Object.entries(product.resources).map(([type, qty]) => {
      ...
      return { type, qty: qty as number, res, available };
    });
  };
                ...
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center overflow-hidden">
                    {isImageIcon(itemDisplay.icon) ? (
                      <img src={itemDisplay.icon} className="w-10 h-10 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                    ) : (
