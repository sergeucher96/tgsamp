...
import { QUESTS_DATABASE, type Quest } from '../gangs/data/questsConfig';
...
interface SelectedQuestData {
  quest: Quest;
  stages: Stage[];
  progress: number;
}

export default function QuestView({ onClose }: QuestViewProps) {
  ...
  const getStages = (quest: Quest): Stage[] => {
  ...
  const selectedData: SelectedQuestData | null = selectedQuest ? {
    quest: selectedQuest,
            ...
            {selectedData ? (
              <div className="p-4">
                ...
                {selectedData.progress >= 100 && !completedQuestIds.includes(selectedData.quest.id) && (
                  <button
                    onClick={() => store.completeQuest(selectedData.quest)}
