...
interface Inherited {
  ...
  effects: { allowed: InheritedEffect[]; denied: InheritedEffect[] };
  actions: InheritedAction[];
  tags: { automatic: InheritedTag[]; manual: InheritedTag[] };
}
...
export default function CategoryEditor({ onClose }: CategoryEditorProps) {
  const {
  ...
  const [mainTab, setMainTab] = useState<'categories' | 'allItems'>('categories');
  ...
  const toggleExpand = (id: string | number) => setExpandedCats(prev => { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; });
  ...
  const openEdit = (cat: Category) => { setName(cat.name); setKey(cat.key); setDescription(cat.description || ''); setIcon(cat.icon || '📂'); setParentId(String(cat.parent_id || '')); setEditingCategory(cat); setShowForm(true); };
  ...
  const renderTree = (cats: Category[]) => cats.map(cat => {
  ...
  if (selectedCategory) {
    return <CategoryDetail
      ...
      properties={properties as any} effects={effects as any} actions={actions as any} tags={tags as any} items={items as any}
    ...
    <div className="fixed inset-0 z-[600] bg-[#020617]/98 backdrop-blur-xl flex flex-col text-white">
      ...
      <div className="flex gap-1 px-4 py-2 border-b border-white/10 overflow-x-auto">
        {[
          ...
          <button key={t.key} onClick={() => setMainTab(t.key as 'categories' | 'allItems')} className={`flex items-center gap-1 px-3 py-1 rounded-lg text-xs font-black whitespace-nowrap ${mainTab === t.key ? 'bg-[#7eff69]/20 text-[#7eff69]' : 'text-slate-400'}`}>
            {t.key === 'categories' ? '📂 ' : '📦 '}{t.label}
          </button>
        ))}
      </div>
      <div className="flex-1 overflow-y-auto p-4">
        {mainTab === 'allItems' && <AllItemsView items={items as any} categories={categories} onClose={onClose} />}
        {mainTab === 'categories' && (
        <>
        {showForm && (
...
function CategoryDetail({ category, onBack, onEdit, onDelete, properties, effects, actions, tags, items, parentOptions }: CategoryDetailProps) {
  ...
  const [itemResources, setItemResources] = useState<Record<string, number>>({});
  ...
  const handleItemIconUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      <div className="flex-1 overflow-y-auto p-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap mb-2">
                      {inhTags.automatic.map((t, i) => (
                        <span key={i} className="text-[10px] px-2 py-1 rounded-lg bg-purple-900/30">{(t as { value?: string }).value || t.name}</span>
                      ))}
                    </div>
                  </div>
                )}
        ...
        {activeTab === 'preview' && (
          <div className="p-4 rounded-2xl border border-[#7eff69]/20 bg-[#0b1b0d]/80">
            ...
            {inhProps.map((p, i) => <p key={i} className="text-xs">• {p.name}</p>)}
          </div>
        )}
      </div>
    </div>
...
function AllItemsView({ items, categories, onClose }: { items: Item[]; categories: Category[]; onClose: () => void }) {
