...
import { RECIPES as DEFAULT_RECIPES } from '../businesses/data/kitchenConfig';
import { useItemCategoryStore } from '../../stores/useItemCategoryStore';
import { CHARACTER_STATS, BUFF_STAT_KEYS } from '../character/data/characterStats';
...
interface Item {
  ...
  action: string;
  value: number;
  effects?: Array<{ effect_key: string; value: number; duration_minutes?: number }>;
}
...
const saveCustomItems = (items: any) => localStorage.setItem('recipe_editor_custom_items', JSON.stringify(items));
...
const saveCustomRecipes = (r: any) => localStorage.setItem('recipe_editor_custom_recipes', JSON.stringify(r));

const TYPE_LABELS: Record<string, string> = { ingredient: '🥘 Ингредиенты', resource: '⛏️ Ресурсы', food: '🍽️ Блюда', tool: '🔧 Инструменты', other: '📦 Прочее' };
...
const mapStoreItem = (item: any): Item => {
...
export default function RecipeEditor({ onClose }: RecipeEditorProps) {
  const { items: storeItems, categories, properties, effects, loadAll, createItem, updateItem } = useItemCategoryStore();
  const [customRecipes, setCustomRecipes] = useState<Recipe[]>(() => loadCustomRecipes().map((r: Recipe) => ({ ...r, source: 'custom' as const })));
  const [defaultRecipes] = useState<Recipe[]>(() => {
    const ids = new Set(loadCustomRecipes().map((r: Recipe) => r.id));
    return (DEFAULT_RECIPES.filter(r => !ids.has(r.id)).map(r => ({ ...r, source: 'default' as const })) as Recipe[]);
  });
  ...
  const [ingredients, setIngredients] = useState<Array<{ itemId: string; amount: number }>>([]);
  ...
  const [itemEffects, setItemEffects] = useState<Record<string, number>>({});
  ...
  const ALL_ITEMS_DB: Record<string, Item> = { ...storeMapped.reduce((acc: Record<string, Item>, item: Item) => ({ ...acc, [item.id]: item }), {}), ...customItems };
  ...
  useEffect(() => {
    if (preparedFoodCategory && !selectedCategoryId) {
      setSelectedCategoryId(Number(preparedFoodCategory.id));
    }
  }, [preparedFoodCategory, selectedCategoryId]);

  const categoryProperties = selectedCategoryId ? properties.filter(p => {
      const link = (window as any).__categoryProperties?.find((cp: any) => cp.category_id === selectedCategoryId && cp.property_id === p.id);
      return !!link;
    }) : [];

  const categoryEffects = selectedCategoryId ? effects.filter(e => {
    const link = (window as any).__categoryEffectsAllowed?.find((ce: any) => ce.category_id === selectedCategoryId && ce.effect_id === e.id);
    return !!link;
  }) : [];
  ...
  const startNew = () => {
    ...
    setSelectedCategoryId(Number(preparedFoodCategory?.id) || null);
    setItemProperties({});
    setItemEffects({} as Record<string, number>);
  ...
  const editRecipe = (r: Recipe) => {
    ...
    const ings: Array<{ itemId: string; amount: number }> = [], counts: Record<string, number> = {};
    ...
    const buffEffect = ci?.effects?.find((e: any) => e.effect_key?.startsWith('buff_'));
    if (buffEffect) {
  ...
  const save = async () => {
    ...
    const flat: string[] = [];
    ingredients.forEach(i => { for (let n = 0; n < i.amount; n++) flat.push(i.itemId); });
    ...
    if (itemAction && effectsList.length === 0) {
      if (itemAction.startsWith('buff_')) {
        effectsList.push({ effect_key: itemAction, value: Number(buffAmount) || 0 } as any);
      } else {
    ...
    let savedItem = null;
    const existing = (storeItems || []).find((i: any) => i.item_key === itemId);
    if (existing) {
      savedItem = await updateItem(existing.id, newItem as any);
    } else {
      savedItem = await createItem(newItem as any);
    }
    ...
    const recipe: Recipe = { id: editIdx !== null ? customRecipes[editIdx].id : `recipe_${Date.now()}`,
      name: recipeName.trim(), icon: itemIcon, ingredients: flat, resultItem: itemId,       resultAmount: Number(resultAmount) || 1, source: 'custom' };
    const cr = loadCustomRecipes();
    if (editIdx !== null) cr[editIdx] = recipe; else cr.unshift(recipe);
    saveCustomRecipes(cr);
    setCustomRecipes([...loadCustomRecipes()].map(r => ({ ...r, source: 'custom' as const })));
    await loadAll();
  ...
  const del = (id: string) => {
    ...
    const cr = loadCustomRecipes().filter((r: Recipe) => r.id !== id); saveCustomRecipes(cr);
    setCustomRecipes(cr.map(r => ({ ...r, source: 'custom' as const })));
  ...
  const dup = (r: Recipe) => {
    ...
    setCustomRecipes([...loadCustomRecipes()].map(x => ({ ...x, source: 'custom' as const })));
    say('📋 Скопирован');
  };
            ...
            <div className="bg-purple-500/5 border border-purple-500/20 rounded-2xl p-4 space-y-3">
                  ...
                  <div className="space-y-1">
                    {categoryEffects.map(eff => (
                      <div key={eff.id} className="flex items-center gap-2">
                        ...
                        <input type="number" value={itemEffects[eff.key] || 0} onChange={e => setItemEffects(prev => ({ ...prev, [eff.key]: Number(e.target.value) }))} placeholder="Значение" className="flex-1 bg-black/50 border border-white/10 rounded-lg px-2 py-1 text-xs" />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <div><label className="text-[10px] font-black uppercase text-slate-400">Стакаемый</label>
                  <select value={itemStackable ? 'true' : 'false'} onChange={e => setItemStackable(e.target.value === 'true')} className="w-full bg-white/5 border border-white/10 rounded-xl py-2 px-2 text-xs text-white font-black uppercase focus:outline-none">
