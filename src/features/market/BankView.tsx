...
import { HOUSE_CLASSES } from '../houses/data/houseConfig';
import { useHouseStore } from '../../stores/useHouseStore';
import { BUSINESS_TYPES } from '../businesses/data/businessConfig';
...
export default function BankView({ onClose }: BankViewProps) {
  ...
  const [isPayingHouseTax, setIsPayingHouseTax] = useState<string | null>(null);
  ...
  const handlePayTax = async (customAmount?: string) => {
  ...
  const handlePayHouseTax = async (houseId: string) => {
                      ...
                      {player && dbHouses && dbHouses.filter(h => h.owner_id === player.id).length > 0 ? (
                        dbHouses
                          ...
                          .map((house, idx) => {
                            ...
                            const taxPaidUntil = (house as any).tax_paid_until ? new Date((house as any).tax_paid_until) : null;
                            const isTaxPaid = taxPaidUntil && taxPaidUntil > new Date();
                            const daysLeft = isTaxPaid ? Math.ceil((taxPaidUntil.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : 0;
              ...
              {activeTab === 'tax' && (
                <div className="grid grid-cols-1 gap-4 mb-4">
                             ...
                             return (
                               <div key={idx} className="flex flex-col gap-1 p-2 bg-white/5 rounded-lg">
                                 <div className="flex justify-between text-[11px]">
                                   <span className="text-slate-400">Дом #{(house as any).id?.replace('h_', '') || house.id_name} ({houseClass.name})</span>
                        ...
                        businesses
                          ...
                          .map((biz, idx) => {
                            const bizTax = Math.round((biz.daily_earnings || 0) * 0.02);
                             const bizType = String(biz.id).split('_')[0];
                  ...
                  <div className="bg-white/[0.03] border border-white/6 p-4 rounded-2xl">
                    ...
                    />
                    <button 
                      onClick={() => handlePayTax(String(totalTax))} 
              ...
              {activeTab === 'history' && (
                <div className="mb-4">
                  {transactions.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {transactions.map((tx: { id: string; type: string; description?: string; amount: string | number; created_at: string }) => (
