...
import BankView from '../market/BankView';
import { OrganizationPanel } from '../gangs/OrganizationView';
import { 
...
function AntennaSignal({ bars = 4 }: { bars?: number }) {
  return (
...
function BatteryGauge({ percent = 85 }: { percent?: number }) {
...
export default function PhoneView({ onClose }: PhoneViewProps) {
  const { player } = usePlayerStore();
  const { messages, unread, fetchMessages, sendSms, markAsRead } = useSmsStore();
  
  const [currentApp, setCurrentApp] = useState<string>('menu');
