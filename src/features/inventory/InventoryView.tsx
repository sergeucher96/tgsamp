...
import { ITEM_DATABASE } from '../inventory/data/items';
import { CLOTHING_DATABASE } from '../character/data/clothingConfig';
import { RESOURCE_PRICES } from '../jobs/data/economy';
...
{selectedItem && (
        <ItemActionMenu 
          ...
          onUse={async (it: InventoryItem) => { await consumeItem(it); setSelectedItem(null); }}
          onDrop={(it: InventoryItem) => { if(window.confirm("Выбросить предмет?")) removeItem(it.id, it.amount); setSelectedItem(null); }}
        />
      )}
      ...
      <InventoryGrid 
        ...
        onAction={(it: InventoryItem) => setSelectedItem(it)}
        label={null}
      />
