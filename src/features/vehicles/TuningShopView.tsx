...
import { useVehicleStore, calculateEffectiveSpeed, calculateEffectiveAcceleration, calculateEffectiveHandling, type Vehicle } from '../../stores/useVehicleStore';
import { TUNING_CONFIG, VEHICLE_DATABASE, HEALTH_PENALTIES, REPAIR_COST_PER_PERCENT } from '../vehicles/data/vehicleConfig';

interface TuningShopViewProps {
  onClose: () => void;
}

export default function TuningShopView({ onClose }: TuningShopViewProps) {
  ...
  const config = (VEHICLE_DATABASE[selectedVehicle.model_id] || {}) as { name?: string; [key: string]: unknown };
  ...
  const getStageInfo = (part: string, currentStage: number) => {
  ...
  const getNextStage = (part: string) => {
    if (part === 'nitro') return null;
    const cfg = TUNING_CONFIG[part];
    if (!cfg) return null;
    const currentStage = (selectedVehicle[`${part}_stage`] || 0) as number;
    return cfg.stages[currentStage] || null;
  };

  const handleTune = async (part: string) => {
    if (part === 'nitro') {
      if (selectedVehicle.has_nitro) return;
      if (Number(player.money) < TUNING_CONFIG.nitro.price) return;
      await tuneVehicle(selectedVehicle.id, part as keyof typeof TUNING_CONFIG, 1);
    } else {
      const next = getNextStage(part);
      if (!next) return;
      if (Number(player.money) < next.price) return;
      await tuneVehicle(selectedVehicle.id, part as keyof typeof TUNING_CONFIG, next);
    }
  };
    ...
    <div className="fixed inset-0 z-[300] bg-[#050814] flex flex-col text-white overflow-y-auto no-scrollbar">
      ...
      <div className="px-6 mb-4">
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {myVehicles.map(v => {
            const vConfig = (VEHICLE_DATABASE[v.model_id] || {}) as { name?: string; [key: string]: unknown };
            ...
            return (
              ...
              >
                ...
                <div className="text-[10px] font-black truncate">{(config as { name?: string }).name || v.model_id}</div>
      ...
      <div className="px-6 pb-8">
        {activeTab !== 'nitro' && (
          <>
            {(() => {
              const currentStage = (selectedVehicle[`${activeTab}_stage`] || 0) as number;
