...
import { VEHICLE_DATABASE, WEAR_SYSTEMS, WEAR_SYSTEM_ORDER, DIAGNOSTIC_COST, WearSystemKey } from '../vehicles/data/vehicleConfig';
import { getDiagnosis, getStatusText, calculateOverallCondition, getPerformanceMultiplier } from '../vehicles/utils/vehicleWear';
...
interface AutoServiceViewProps {
  onClose: () => void;
}

export default function AutoServiceView({ onClose }: AutoServiceViewProps) {
  ...
  const showMsg = (msg: string, type = 'info') => {
  ...
  const handleService = async (systemKey: string) => {
    if (!selectedVehicle) return;
    ...
    setProcessing(true);
    const success = await serviceWearSystem(selectedVehicle.id, systemKey as WearSystemKey);
    setProcessing(false);
    if (success) {
      ...
      <div className="w-full flex-1 overflow-y-auto">
        <div className="max-w-lg mx-auto pt-8 px-6">
          ...
          <div className="mb-6 flex gap-2 overflow-x-auto no-scrollbar pb-2">
            {(myVehicles || []).map(v => {
              const vCfg = (VEHICLE_DATABASE[v.model_id] || {}) as { name?: string; [key: string]: unknown };
