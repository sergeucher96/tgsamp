...
import { type HouseClass } from '../../features/houses/data/houseConfig';
      ...
      gltf.scene.traverse((child: THREE.Object3D) => {
        ...
        if (mesh.isMesh && mesh.material) {
          if (Array.isArray(mesh.material)) {
            mesh.material.forEach((mat: THREE.Material) => (mat as THREE.MeshStandardMaterial).color.set(color));
          } else {
...
function Lights() {
  return (
    <>
      ...
      <hemisphereLight color="#87ceeb" groundColor="#080c14" intensity={0.3} />
    </>
  );
}

function CarMiniScene({ model, color, autoRotate = true }: { model: string; color: string; autoRotate?: boolean }) {
  return (
    <Canvas
...
interface CarShowroomProps {
  onClose: () => void;
  playerHouses: Array<{ id_name: string; name: string; class: HouseClass }>,
  playerPos: { x: number; y: number };
  showroomPos: { x: number; y: number };
}

export default function CarShowroom({ onClose, playerHouses, playerPos, showroomPos }: CarShowroomProps) {
  const models = Object.keys(VEHICLE_DATABASE).filter(m => VEHICLE_DATABASE[m].price > 0);
  const [modelIndex, setModelIndex] = useState(0);
  const [selectedColor, setSelectedColor] = useState('white');
  const [selectedHouseId, setSelectedHouseId] = useState<string | null>(playerHouses[0]?.id_name || null);
