...
import { useHotelStore, type HotelState, type HotelRoom } from '../../stores/useHotelStore';
import { HOTEL_PURCHASE_PRICE, HOTEL_ROOMS, HOTEL_PRICES, HOTEL_PRICE_LABELS } from '../businesses/data/hotelConfig';
...
export default function HotelView({ hotelId, onClose }: HotelViewProps) {
  ...
  const now = new Date();
  const activeRooms = state.rooms.filter((r: HotelRoom) => r.rented && r.expires_at && new Date(r.expires_at) > now);
  const freeRooms = HOTEL_ROOMS - activeRooms.length;
  const hotelInfo = FINAL_LOCATIONS?.find((l: Location) => l.id === hotelId);
  const hotelName = hotelInfo?.name || String(state.id);
              ...
              {activeRooms.length > 0 && (
                <div className="mt-3">
                  ...
                  <div className="space-y-2 max-h-40 overflow-y-auto">
                    {activeRooms.map((room: HotelRoom, i: number) => (
