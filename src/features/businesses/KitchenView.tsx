...
const loadCustomItems = (): Record<string, ItemData> => {
  try { const s = localStorage.getItem('recipe_editor_custom_items'); return s ? JSON.parse(s) : {}; }
  catch { return {}; }
};

const ITEM_DATABASE = { ...DEFAULT_ITEMS, ...loadCustomItems() } as unknown as Record<string, ItemData>;
...
export default function KitchenView({ onClose, houseId }: KitchenViewProps) {
  ...
  const [resultItem, setResultItem] = useState<ItemData | null>(null);
  ...
  const allRecipes = useCallback((): Recipe[] => {
    try {
      ...
      const custom: Recipe[] = saved ? JSON.parse(saved) : [];
  ...
  const placeIngredient = (item: InventoryItem, source: 'player' | 'house') => {
  ...
  const removeIngredient = (slotIndex: number) => {
    setSlots(prev => {
      const next = [...prev];
      next[slotIndex] = null;
      return next;
  ...
  const checkRecipe = useCallback(() => {
    ...
    const filledSlots = slots.filter((s): s is SlotItem => s !== null).map(s => s.item_id).sort();
  ...
  const cook = async () => {
    ...
    const filledSlots = slots.filter((s): s is SlotItem => s !== null);
  ...
  const getCurrentItemAmount = (slot: SlotItem): number => {
    const item = [...playerItems, ...houseInvItems].find(i => i.id === slot.id);
    return item?.amount || 1;
  };

  const finishCooking = async (recipe: Recipe, usedSlots: SlotItem[]) => {
    ...
    for (const slot of usedSlots) {
      const { error } = await supabase
        .from('inventory')
        .update({ amount: Math.max(0, (parseInt(getCurrentItemAmount(slot).toString()) - 1)) })
        .eq('id', slot.id)
        .gt('amount', 0);
      ...
      {showRecipeEditor ? (
        ...
        <>
          ...
          <div className="flex-1 flex flex-col items-center justify-center gap-8 px-6 pt-20 pb-6">
            ...
            {/* Recipe hint */}
            {slots[0] && ITEM_DATABASE[slots[0].item_id]?.id && ITEM_DATABASE[slots[0].item_id]?.name && (
                  ...
                  return (
                    ...
                    >
                      {isImageIcon(itemData?.icon) ? (
                        <img src={itemData.icon} className="w-8 h-8 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      ) : (
