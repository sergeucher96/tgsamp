...
import { useWeaponStore, type WeaponType, type WeaponState, type WeaponsMap } from '../../stores/useWeaponStore';
import { WEAPON_CONFIG, GUN_RANGE_SETTINGS, WEAPON_LICENSE_QUESTIONS, type WeaponConfig, type QuizQuestion } from '../market/data/weaponConfig';

const STAGE = { MENU: 'menu', GAME: 'game', SHOP: 'shop', LICENSE_TEST: 'license_test', RESULT: 'result' } as const;
type StageType = typeof STAGE[keyof typeof STAGE];

const targetSizeClass: Record<string, string> = {
  small: 'w-10 h-10',
  medium: 'w-16 h-16',
  large: 'w-20 h-20',
};

interface GunRangeViewProps {
  onClose: () => void;
}

export default function GunRangeView({ onClose }: GunRangeViewProps) {
  ...
  const [targets, setTargets] = useState<Array<{ id: number; x: number; y: number; lifetime: number }>>([]);
  const [gameActive, setGameActive] = useState(false);
  const [totalHits, setTotalHits] = useState(0);
  const [totalShots, setTotalShots] = useState(0);
  const [gameResult, setGameResult] = useState<{ score: number; totalHits: number; totalShots: number } | null>(null);

  // License test
  const [testQuestions, setTestQuestions] = useState<QuizQuestion[]>([]);
  const [currentTestQ, setCurrentTestQ] = useState(0);
  const [testScore, setTestScore] = useState(0);
  const [testResult, setTestResult] = useState<{ passed: boolean; score?: number; error?: boolean } | null>(null);

  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const targetTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const idCounter = useRef(0);

  const config: WeaponConfig | null = selectedWeapon ? WEAPON_CONFIG[selectedWeapon] : null;
  const weaponState: WeaponState | null = selectedWeapon ? weapons[selectedWeapon] : null;
  ...
  const startSpawningTargets = useCallback(() => {
    const spawn = () => {
      ...
      targetTimerRef.current = setTimeout(spawn, nextDelay);
    };
    spawn();
  ...
  // --- START GAME ---
  const startGame = async (weaponType: WeaponType) => {
  ...
  const purchaseWeapon = async (weaponType: WeaponType) => {
    if (!weaponLicense) {
  ...
  const answerLicenseTest = (idx: number) => {
        ...
        {stage === STAGE.MENU && (
          <div className="space-y-3">
            ...
            {Object.values(WEAPON_CONFIG).map(wep => {
              ...
              return (
                <button key={wep.id} onClick={() => owned ? startGame(wep.id as WeaponType) : openShop()}
              ...
              return (
                <div key={wep.id} className={`bg-slate-900/50 border p-5 rounded-[28px] ${owned ? 'border-green-500/20' : 'border-white/5'}`}>
                  ...
                  {owned ? (
                    ...
                    <button onClick={() => purchaseWeapon(wep.id as WeaponType)} disabled={!weaponLicense}
