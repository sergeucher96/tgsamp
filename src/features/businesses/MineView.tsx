...
import { MINING_LOOT, type MiningLoot, MINING_SETTINGS } from '../businesses/data/miningConfig';
import { ITEM_DATABASE, type ItemData } from '../inventory/data/items';
import { Zap, X, Pickaxe } from 'lucide-react';

export default function MineView({ onClose }: { onClose: () => void }) {
  const { player, updateProfile, skills } = usePlayerStore();
  const { items, buyItem } = useInventoryStore();

  const [isMining, setIsMining] = useState(false);
  const [progress, setProgress] = useState(0);
  const [lastLoot, setLastLoot] = useState<ItemData | null>(null);

  const miningSkill = (skills || []).find((s: Skill) => s.skill_name === 'miner')?.value || 0;
  const hasPickaxe = (items || []).some((i) => i.item_id === 'pickaxe');

  const calculateLoot = (): MiningLoot => {
