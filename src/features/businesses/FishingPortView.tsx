...
interface FishingPortViewProps {
  onClose: () => void;
}

export default function FishingPortView({ onClose }: FishingPortViewProps) {
  const { player, updateProfile } = usePlayerStore();
  const { items, buyItem } = useInventoryStore();

  const [isFishing, setIsFishing] = useState(false);
  const [progress, setProgress] = useState(0);
  const [lastCatch, setLastCatch] = useState<CatchData | null>(null);
  const [phase, setPhase] = useState<'idle' | 'casting' | 'waiting' | 'reeling'>('idle');

  const hasFishingRod = (items || []).some((i: InventoryItem) => i.item_id === 'fishing_rod');
  ...
  const startFishing = () => {
    ...
    setTimeout(() => {
      const roll = Math.random() * 100;
      let catchData: CatchData | null = null;
