...
const initialDancers: Dancer[] = [
  ...
  },
  {
    ...
    description: 'Сильный сценический образ и яркая энергетика. Её присутствие вдохновляет на большие сделки и бонусы.',
...
interface StripClubViewProps {
  onClose: () => void;
}

export default function StripClubView({ onClose }: StripClubViewProps) {
  const player = usePlayerStore((state) => state.player);
  const [dancers, setDancers] = useState<Dancer[]>(initialDancers);
  const [selectedDancerId, setSelectedDancerId] = useState<string>(initialDancers[0].id);
  const [touchStartX, setTouchStartX] = useState<number | null>(null);

  const handleImproveReputation = (id: string) => {
                ...
                <div className="rounded-[28px] border border-[#7eff67]/10 bg-[#0b140f] p-3">
                  <div className="h-56 rounded-[28px] bg-[#112113] overflow-hidden mb-3 flex items-center justify-center">
                    <img
                      ...
                      onError={(e) => { (e.target as HTMLImageElement).onerror = null; (e.target as HTMLImageElement).src = '/stripclub/placeholder.png'; }}
                    />
                  </div>
