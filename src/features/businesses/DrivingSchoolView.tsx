...
import { useWeaponStore, type LicenseType, type DrivingExamAttempts } from '../../stores/useWeaponStore';
import { LICENSES_DATABASE, type License } from '../vehicles/data/licenses';
import { DRIVING_TEST_QUESTIONS, type QuizQuestion } from '../market/data/weaponConfig';

const shuffle = <T,>(arr: T[]): T[] => [...arr].sort(() => Math.random() - 0.5);

const DRIVING_LICENSES: License[] = LICENSES_DATABASE.filter(l => ['moto', 'car', 'truck'].includes(l.id));

const STAGE = { MENU: 'menu', QUIZ: 'quiz', DRIVING_GAME: 'driving', RESULT: 'result' } as const;
type StageType = typeof STAGE[keyof typeof STAGE];

interface DrivingSchoolViewProps {
  onClose: () => void;
}

export default function DrivingSchoolView({ onClose }: DrivingSchoolViewProps) {
  ...
  const [examResult, setExamResult] = useState<{ passed: boolean; message: string } | null>(null);

  // Мини-игра вождения
  const [drivingScore, setDrivingScore] = useState(0);
  const [gameTime, setGameTime] = useState(30);
  const [gameActive, setGameActive] = useState(false);
  const [obstacle, setObstacle] = useState<{ action: 'brake' | 'turn'; lane: number } | null>(null);
  const [actionRequired, setActionRequired] = useState<'brake' | 'turn' | null>(null);
  ...
  const startExam = async (licenseId: LicenseType) => {
  ...
  const handleDrivingAction = (actionType: 'brake' | 'turn') => {
  ...
  const finishExam = async (passed?: boolean, message?: string) => {
    clearTimers();
    setGameActive(false);
    gameActiveRef.current = false;
    if (passed === undefined) {
    ...
    if (passed) await getDrivingLicense(selectedLicense as LicenseType);
    setExamResult({ passed: passed as boolean, message: message as string });
        ...
        {stage === STAGE.MENU && (
          <div className="space-y-3">
            ...
            {DRIVING_LICENSES.map(lic => {
              const hasLic = hasDrivingLicense(lic.id as LicenseType);
              const att = drivingExamAttempts[lic.id as keyof DrivingExamAttempts] || 0;
              return (
                <button key={lic.id} onClick={() => startExam(lic.id as LicenseType)} disabled={hasLic}
