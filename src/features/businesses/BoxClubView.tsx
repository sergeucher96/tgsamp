...
import { usePlayerStore, type Profile, type Skill } from '../../stores/usePlayerStore';
...
export const createFighter = (base: Partial<Fighter> & { id: string; name: string; maxHp: number }): Fighter => ({
  id: base.id,
  name: base.name,
  maxHp: base.maxHp,
  hp: base.maxHp,
...
export const calculateDamage = (attacker: Fighter, defender: Fighter, skillValue = 0): number => {
...
export const calculateHeal = (fighter: Fighter, skillValue = 0): number => {
...
// Состояния боя
// ==========================================
export const STAGE = { MENU: 'menu', FIGHT: 'fight', RESULT: 'result', LEADERBOARD: 'leaderboard' } as const;
...
const loadLeaderboard = (): LeaderboardRecord[] => {
  try {
...
const addLeaderboardRecord = (playerName: string, opponentName: string, won: boolean, boxingExp: number): LeaderboardRecord[] => {
...
export default function BoxClubView({ onClose }: BoxClubViewProps) {
  ...
  const getSkillValue = useCallback(() => {
    const skill = (skills || []).find((s: Skill) => s.skill_name === 'boxing');
  ...
  const startFight = (opponentTemplate: OpponentTemplate) => {
  ...
  const getOpponentAction = (fighter: Fighter): Action => {
  ...
  const executePlayerAction = (action: Action) => {
  ...
  const endFight = async (result: 'win' | 'lose', finalPlayer: Fighter, finalOpponent: Fighter) => {
    clearAllTimers();
