...
import { ORGANIZATIONS, CRIMINAL_ORGANIZATIONS } from '../gangs/data/organizationsConfig';
import { WAR_STATUSES, EVENT_TYPES, EVENT_RESULTS } from '../../stores/useWarStore';
import { getAvailableActions } from '../../game/world/warScoring';

interface WarsViewProps { onClose: () => void; territoryId?: string; }

export default function WarsView({ onClose, territoryId }: WarsViewProps) {
  const {
    wars,
    events,
    participants,
  ...
  const handleEndWar = async (warId) => {
    ...
    const ok = await endWar(warId, 'DRAW');
    if (ok) {
