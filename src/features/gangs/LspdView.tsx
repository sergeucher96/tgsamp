...
interface LspdViewProps { onClose: () => void; }

export default function LspdView({ onClose }: LspdViewProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const timers = {};
      cameras.forEach(cam => {
        const remaining = Math.max(0, Math.ceil((new Date(cam.expires_at).getTime() - Date.now()) / 1000));
  ...
  const handleStartPatrol = (routeId) => {
    ...
    if (routeId) {
      ...
      startPatrol(selectedPatrolType as 'foot' | 'vehicle', null, route);
    }
    setPatrolMenu(false);
    setPatrolStep('type');
    setSelectedRoute('');
