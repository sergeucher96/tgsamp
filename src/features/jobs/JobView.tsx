...
import { JOBS_DATABASE, type JobTask, type RouteJob, type StationJob, type GarbageJob } from '../jobs/data/jobsConfig';
import { useJobStore, type ActiveShift, type RouteShift, type GarbageShift } from '../../stores/useJobStore';
...
export default function JobView({ jobId, onClose }: JobViewProps) {
  ...
  const stop = shift && shift.kind === 'route' ? shift.stops[shift.currentStop] : undefined;
...
function RouteBody({ job, shift, stop, busy, onStart, onGo, onComplete, onReturn, onCancel, disabled }: RouteBodyProps) {
  if (!shift) {
    return (
  ...
  const routeShift = shift as RouteShift;
  const done = routeShift.status === 'toBase' || routeShift.status === 'returning';

  return (
    <div className="space-y-4">
      {routeShift.cargo && (
        <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-4 text-sm text-slate-300">Груз: <b>{routeShift.cargo}</b></div>
      )}

      <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-5 space-y-3">
        {routeShift.stops.map((s, index) => {
          const isCurrent = index === routeShift.currentStop && !done;
          const isPassed = index < routeShift.currentStop || done;
          return (
...
interface GarbageBodyProps {
  ...
  onReturn: () => void;
  onCancel: () => void;
  onGo: (binId: string) => Promise<void>;
  onStart: () => void;
  disabled: boolean;
}

function GarbageBody({ job, shift, busy, onCollect, onReturn, onCancel, onGo, onStart, disabled }: GarbageBodyProps) {
  if (!shift) {
    return (
      <div className="space-y-4">
        <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-5">
  ...
  const garbageShift = shift as GarbageShift;
  const capacity = garbageShift.capacity || 0;
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-5 space-y-3">
        ...
        {garbageShift.lastBinAmount > 0 && shift.status === 'at_bin' && !shift.collecting && (
          <p className="text-xs text-slate-400">В этом контейнере: {garbageShift.lastBinAmount} кг</p>
        )}
      </div>

      <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-5 space-y-3">
        <p className="text-[10px] uppercase tracking-[0.3em] text-slate-500 font-black mb-2">Контейнеры</p>
        {garbageShift.activeBins && garbageShift.activeBins.map((binId, index) => {
          const isCurrent = binId === garbageShift.selectedBinId && shift.status !== 'driving_to_base';
          const isPassed = binId !== garbageShift.selectedBinId && shift.status === 'driving_to_base';
          return (
...
function StationBody({ job, shift, busy, progress, lastTask, tasks, onStart, onRun, onEnd, disabled }: StationBodyProps) {
  if (!shift) {
    return (
      <div className="space-y-4">
        <div className="bg-white/[0.03] border border-white/10 rounded-[32px] p-5">
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3 text-center">
        <Stat icon={<Wrench size={14} className="text-sky-400" />} label="Нарядов" value={String(shift.tasksDone)} />
