...
import { useTruckerStore, TRUCK_TYPES, BUY_PRICE, SELL_PRICE } from '../../stores/useTruckerStore';
...
interface TruckerViewProps {
  onClose: () => void;
}

export default function TruckerView({ onClose }: TruckerViewProps) {
  ...
  const handleRent = async (truckId: string) => {
    const success = await rentTruck(truckId);
    if (success) showMessage('✅ Транспорт арендован!');
  };

  const handleBuy = async (resourceType: 'crop' | 'metal' | 'oil') => {
                        ...
                        <div key={order.id} className="bg-white/[0.03] border border-white/5 p-5 rounded-[32px]">
                          <div className="flex items-center gap-3 mb-3">
                            ...
                            <div className="text-right">
                              <p className="text-sm font-black text-amber-400">{Math.round(Number(order.quantity))} ед.</p>
