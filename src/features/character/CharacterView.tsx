...
export default function CharacterView({ onClose }: CharacterViewProps) {
  ...
  const handleEquip = async (item_id: string) => {
  ...
  const wearableItems = items.filter((item: InventoryItem) => CLOTHING_DATABASE[item.item_id]);
  const activeBuffsList = (activeBuffs || []).filter((b: Buff) => b.expiresAt > now);
    ...
    <div className="fixed inset-0 z-[300] bg-[#050814] flex flex-col text-white overflow-y-auto no-scrollbar">
      ...
      <div className="px-6 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {CHARACTER_STATS.filter((s) => s.key !== 'inv_slots').map((stat: CharacterStat) => {
            const statData = CHARACTER_STATS_MAP[stat.key];
            return (
              <div key={stat.key} className={`${statData.bg}/10 border ${statData.bg}/20 rounded-2xl p-4 text-center`}>
                <div className="text-lg mb-1">{statData.icon}</div>
                <div className={`text-[9px] uppercase text-slate-400 font-black`}>{statData.name}</div>
                <div className={`text-xl font-black ${statData.color}`}>+{stats[stat.key] || 0}</div>
              </div>
            );
          })}
        </div>
      ...
      <div className="px-6 mb-6">
        ...
        <div className="space-y-2">
          {Object.entries(EQUIPMENT_SLOTS).map(([slotId, slotInfo]) => {
            const equippedItem_id = equipment[slotId as EquipmentSlot];
            ...
            return (
              <div key={slotId} className="bg-white/5 border border-white/10 rounded-2xl p-4">
                <div className="flex items-center justify-between">
                  ...
                  {equippedItem && (
                    <button
                      onClick={() => handleUnequip(slotId as EquipmentSlot)}
      ...
      {wearableItems.length > 0 && (
        <div className="px-6 pb-8">
          ...
          <div className="grid grid-cols-2 gap-2">
            {wearableItems.map((item: InventoryItem) => {
              const itemData = CLOTHING_DATABASE[item.item_id] as ClothingItem | undefined;
