...
import { SKILLS_DATABASE, type Skill } from '../character/data/skills';
import { LICENSES_DATABASE } from '../vehicles/data/licenses';
import { getHouseStyle } from '../houses/data/houseStyles';
import type { Profile, Skill as PlayerSkill, License } from '../../stores/usePlayerStore';
...
function getLicenseInfo(licenses: License[], id: string): LicenseInfo {
  const lic = licenses.find((l) => l.license_type === id);
  if (!lic) return { active: false, text: 'Нет' };
  const diff = new Date(lic.expires_at).getTime() - Date.now();
  if (diff <= 0) return { active: false, text: 'Истекла' };
  const days = Math.floor(diff / 86400000);
  return { active: true, text: days > 0 ? `${days} дн.` : 'Скоро истечет' };
}

function getTimeInState(player: Profile): string {
  if (!player.registered_at) return "1-й день";
  const days = Math.floor((Date.now() - new Date(player.registered_at).getTime()) / 86400000);
...
function renderSkillBlock(
  ...
  return (
    <div className="bg-slate-900/50 border border-white/5 rounded-[28px] p-5 text-left">
      <h3 className="text-[10px] font-black uppercase text-slate-500 mb-5 flex items-center gap-2">
...
export default function ProfileView({ player, skills, licenses, onOpenCharacter }: ProfileViewProps) {
    ...
    <div className="min-h-full p-5 pb-32 space-y-6 animate-in fade-in slide-in-from-bottom-5">
      ...
      <div className="bg-slate-900 border border-white/5 rounded-[32px] p-6 relative overflow-hidden shadow-2xl">
        <div className="relative z-10 text-left">
          <div className="text-blue-500 font-black uppercase text-[9px] tracking-widest mb-2 flex items-center gap-2">
            <Shield size={12} /> Passport
          </div>
          <h2 className="text-3xl font-black uppercase italic leading-[0.8] tracking-tighter">
            {player.first_name} <br />
            <span className="text-blue-600">{player.last_name}</span>
          </h2>
          <div className="flex gap-3 mt-8">
            <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/5 text-[10px] font-bold uppercase italic">
              <span className="text-slate-500 block text-[7px]">В штате</span>
              {getTimeInState(player)}
            </div>
            <div className="bg-white/5 px-4 py-2 rounded-2xl border border-white/5 text-[10px] font-bold uppercase italic">
              <span className="text-slate-500 block text-[7px]">Пол</span>
              {player.gender === 'male' ? 'M' : 'W'}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-slate-900/50 border border-white/5 rounded-[28px] p-5 text-left">
        ...
        <div className="grid grid-cols-1 gap-2">
          {LICENSES_DATABASE.map((l) => {
            const status = getLicenseInfo(licenses, l.id);
            return (
              <div key={l.id} className="flex items-center justify-between bg-white/5 p-3 rounded-2xl border border-white/5">
                ...
                <div className={`text-[9px] font-black uppercase italic ${status.active ? 'text-green-500' : 'text-slate-600'}`}>
                  {status.text}
                </div>
              </div>
            );
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-slate-900/50 border border-white/5 p-5 rounded-[28px]">
          <div className="flex justify-between text-sm font-black italic mb-2">
