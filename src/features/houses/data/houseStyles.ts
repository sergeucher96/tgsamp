...
import savedHotspots from '../../../game/locations/savedHotspots.json';
    ...
    if (saved) {
      try {
        ...
        for (const [key, value] of Object.entries(data)) {
          if (key.startsWith(cls + '__')) {
            const label = key.substring(cls.length + 2);
            result[label] = { ...(result[label] || {}), ...(value as Record<string, unknown>) };
          }
        }
      } catch (e) {}
    }
...
export function getHouseGarageData(cls) {
  ...
  if (savedHotspots?.[`${cls}__garage`] || (savedHotspots as Record<string, unknown>)?.[`garage`]) {
    return savedHotspots[`${cls}__garage`] || (savedHotspots as Record<string, unknown>)[`garage`] as any;
  }
