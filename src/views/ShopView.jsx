    ...
    <div className="fixed inset-0 z-[400] bg-[#020617] flex flex-col p-6 text-white font-sans animate-in fade-in duration-300">
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar space-y-3 pb-10">
        {shopData.items.map((shopItem) => {
          const itemInfo = ITEM_DATABASE[shopItem.id];
          if (!itemInfo) return null; // Тихо пропускаем, если предмета нет в каталоге

          return (
            <div key={shopItem.id} className="bg-slate-900/50 border border-white/5 p-5 rounded-[32px] flex items-center justify-between transition-all active:bg-blue-600/5">
              ...
              <button onClick={() => handleBuy(shopItem)} className="bg-blue-600 hover:bg-blue-500 px-5 py-3 rounded-2xl font-black italic text-sm shadow-lg active:scale-90 transition-all">
                ${shopItem.price}
              </button>
            </div>
          );
