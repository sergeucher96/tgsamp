...
import { useTravelStore, type Location } from './useTravelStore';
...
const BASE_ROUTES: BusRoute[] = [];

const BUS_DEPOT = { x: 5252, y: 4982 };
const BONUS_PER_ROUTE = 50;
const STOP_DURATION = 10000;

async function loadBusRoutes(): Promise<BusRoute[]> {
  let customSupabase: BusRoute[] = [];
  try {
    ...
    if (data) customSupabase = data.map(r => {
      ...
      return {
        ...
        busStops: typeof busStops === 'object' && !Array.isArray(busStops) ? busStops : {}
      };
    });
...
interface BusState {
  ...
  sessionRoutesCompleted: number;
  currentStopName: string;
  nextStopName: string;
  message: string | null;
  routeTimer: ReturnType<typeof setTimeout> | null;
  checkCancelInterval: ReturnType<typeof setInterval> | null;
  showRoutePopup: boolean;
  currentStopIndex: number;
  totalStops: number;

  getAvailableRoutes: () => Promise<BusRoute[]>;
  startSession: (routeId: string | number) => Promise<boolean>;
  endSession: () => void;
  startRoute: (routeId: string | number) => Promise<boolean>;
  completeRoute: () => void;
  repeatRoute: () => Promise<boolean>;
  dismissRoutePopup: () => void;
}
...
export const useBusStore = create<BusState>((set, get) => ({
  ...
  startRoute: async (routeId) => {
    ...
    let cancelled = false;
    const getStopName = (r: BusRoute, idx: number) => { const wpId = r.stops[idx]; return (r.busStops && r.busStops[wpId]) || `Точка ${idx + 1}`; };
    ...
    const travel = async () => {
      try {
        ...
        for (let i = 0; i < route.stops.length - 1; i++) {
          ...
          let pts: { x: number; y: number }[], lens: number[], ids: string[];
...
async function animRoute(pts: { x: number; y: number }[], ids: string[], lens: number[], speed: number, totalDist: number) {
  return new Promise<void>(resolve => {
    ...
    let sr = rot0, ls = -1;
    function norm(a: number) { while(a>180)a-=360; while(a<-180)a+=360; return a; }
