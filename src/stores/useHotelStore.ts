...
import { HOTEL_PURCHASE_PRICE, HOTEL_ROOMS, HOTEL_PRICES } from '../features/businesses/data/hotelConfig';
...
export interface HotelStoreState {
  ...
  getHotelState: (hotelId: string | number) => HotelState;
  getDefaultRooms: (existingRooms?: HotelRoom[]) => HotelRoom[];
  getOwnerEarnings: (hotelId: string | number) => number;
  buyHotel: (hotelId: string | number) => Promise<boolean>;
  rentRoom: (hotelId: string | number, days: number) => Promise<boolean>;
}

const getDefaultRooms = (existingRooms: HotelRoom[] = []): HotelRoom[] => {
  const rooms = Array.from({ length: HOTEL_ROOMS }, (_, i): HotelRoom => ({
    room_number: i + 1,
    rented: false,
    renter_id: null,
    renter_name: null,
...
export const useHotelStore = create<HotelStoreState>((set, get) => ({
  ...
  getHotelState: (hotelId) => {
    ...
    return {
      ...
      rooms: rooms.length === HOTEL_ROOMS ? rooms : getDefaultRooms(rooms),
    };
  },

  getDefaultRooms: (existingRooms = []) => getDefaultRooms(existingRooms),
    ...
    try {
      ...
      if (hotelState.owner_id && hotelState.owner_id !== player.id) {
        ...
        if (ownerProfile) {
          await supabase
            .from('profiles')
            .update({ money: Number(ownerProfile.money || 0) + profit })
            .eq('id', hotelState.owner_id);
        }
      }
