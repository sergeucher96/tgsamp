...
export interface CafeteriaStoreState {
  businessCrops: Record<string, number>;
  loading: boolean;

  fetchBusinessCrops: (businessId: string) => Promise<number>;
  getCropCount: (businessId: string) => number;
  decrementCrops: (businessId: string, amount: number) => Promise<boolean>;
  addCrops: (businessId: string, amount: number) => Promise<boolean>;
}

export const useCafeteriaStore = create<CafeteriaStoreState>((set, get) => ({
  businessCrops: {},
  loading: false,
