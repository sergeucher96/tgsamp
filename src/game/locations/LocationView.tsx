...
import {
  ...
  getLocationMusic,
} from '../../features/businesses/data/locationStyles';
...
interface SubLocationStackItem {
  image: string | null;
  hotspots: Hotspot[];
  label: string;
}
...
export default function LocationView({ location, onClose, onAction }: LocationViewProps) {
  ...
  const [subLocationStack, setSubLocationStack] = useState<SubLocationStackItem[]>([]);
  const [currentSubLocation, setCurrentSubLocation] = useState<Hotspot | null>(null);
