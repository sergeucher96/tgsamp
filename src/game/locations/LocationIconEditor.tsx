...
import { loadHouseIcons, saveHouseIcon, resetHouseIcon, resetAllHouseIcons } from '../../features/houses/data/houseStyles';

interface LocationIconEditorProps {
  onClose: () => void;
}
 ...
 */
function compressImageBase64(base64: string, maxDim = 96, quality = 0.85): Promise<string> {
...
export default function LocationIconEditor({ onClose }: LocationIconEditorProps) {
  const [icons, setIcons] = useState<Record<string, string>>({});
  const [houseIcons, setHouseIcons] = useState<Record<string, string>>({});
  const [saving, setSaving] = useState(false);
  const [copied, setCopied] = useState(false);
  const [search, setSearch] = useState('');
  const [previews, setPreviews] = useState<Record<string, string | null>>({});
  const [housePreviews, setHousePreviews] = useState<Record<string, string | null>>({});
  const [errors, setErrors] = useState<Record<string, boolean>>({});
  const [houseErrors, setHouseErrors] = useState<Record<string, boolean>>({});
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, locId: string) => {
  ...
  const handleTextChange = (locId: string, value: string) => {
  ...
  const handleHouseFileChange = (key: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const handleHouseTextChange = (key: string, value: string) => {
  ...
  const handleResetLocationOne = async (locId: string) => {
  ...
  const handleResetHouseOne = async (key: string) => {
