...
} from '../../features/houses/data/houseStyles';
import { VEHICLE_DATABASE } from '../../features/vehicles/data/vehicleConfig';
import KitchenView from '../../features/businesses/KitchenView';
...
interface GarageData {
  image?: string;
  hotspots?: Hotspot[] | Record<string, Hotspot>;
}
...
export default function HouseInterior() {
  ...
  const [mode, setMode] = useState<'exterior' | 'interior' | 'kitchen' | 'garage' | 'sublocation'>('exterior');
  const [navStack, setNavStack] = useState<Array<{ mode: string; subLocationImage: string | null; subLocationHotspots: Hotspot[]; subLocationLabel: string }>>([]);

  // Sublocations
  const [subLocationImage, setSubLocationImage] = useState<string | null>(null);
  const [subLocationHotspots, setSubLocationHotspots] = useState<Hotspot[]>([]);
  ...
  const [garageHotspots, setGarageHotspots] = useState<Hotspot[]>([]);
  const [garageHoveredHotspot, setGarageHoveredHotspot] = useState<string | null>(null);
  ...
  const houseData = dbHouses.find((h) => h.id_name === currentInterior) as HouseData | undefined;
  ...
  const pointerSamplesRef = useRef<Array<{ x: number; time: number }>>([]);
  ...
  const handleExteriorImageLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
    const target = e.target as HTMLImageElement;
    const nw = target.naturalWidth || 16;
    const nh = target.naturalHeight || 9;
    const ratio = nw / nh;
  ...
  const navigateTo = (newMode: typeof mode) => {
    stopInertia();
    setNavStack((prev) => [
      ...prev,
      {
  ...
  const goBack = () => {
    ...
    setNavStack((prevStack) => {
      ...
      setMode(previous.mode as typeof mode);
  ...
  const handleHotspotClick = (pos: Hotspot) => {
  ...
  const handleGarageHotspotClick = (action: string) => {
  ...
  // РЕНДЕР: ГАРАЖ
  if (mode === 'garage') {
    const garageHsList = (Array.isArray(garageHotspots) ? garageHotspots : Object.values(garageHotspots)) as Hotspot[];
          ...
          {garageImage ? (
            ...
            >
              <img
                ...
                onLoad={(e) => { const img = e.target as HTMLImageElement; setGarageRatio((img.naturalWidth || 16) / (img.naturalHeight || 9)); }}
          ...
          {subLocationImage && (
            ...
            >
              <img
                ...
                onLoad={(e) => { const img = e.target as HTMLImageElement; setSubRatio((img.naturalWidth || 16) / (img.naturalHeight || 9)); }}
                className="w-full h-full object-contain pointer-events-none block"
                draggable={false}
              />
